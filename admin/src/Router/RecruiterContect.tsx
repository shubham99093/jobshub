...
const RecruiterContect = () => {
  const { recruiterContect } = useAdmin();
  return (
    <Wrapper value="contect">
      <div>
        <div className="flex justify-between">
          <h1 className="text-3xl text-gray-600 font-medium">
            Recruiter Contect Detail
          </h1>
          <div className="flex gap-2 text-gray-600 font-semibold">
            ...
            <p>/</p>
            <p className="text-gray-600 font-semibold">Recruiter Contect</p>
          </div>
        </div>
        <div className="mt-8 flex gap-6">
          <div className="w-full p-4 bg-white shadow-md rounded-md flex justify-center items-between flex-col">
            ...
            {recruiterContect && recruiterContect?.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full border-gray-300">
                  ...
                  <tbody>
                    {recruiterContect.map((item, index) => {
                      const seeker =
                        typeof item.rec_id === "object" ? item.rec_id : null;
                      return (
                        <tr
                          key={item._id}
                        ...
                        >
                          ...
                          <td className="p-3">{seeker?.cmp_name || "N/A"}</td>
                          <td className="p-3">{seeker?.cmp_email || "N/A"}</td>
                          <td className="p-3">
...
export default RecruiterContect;
