...
const UpdateIndustry = () => {
  ...
  const [indName, setIndName] = useState(location.state.indName as string);
  ...
  const handleAddindustry = async () => {
    ...
    const updatedIndustry = industry?.map((item) => {
      if (item._id === result.data._id) {
        return { ...item, ind_name: indName };
      }
      return item;
    });

    setIndustry(updatedIndustry as IIndustrys[]);
